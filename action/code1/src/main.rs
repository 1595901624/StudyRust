...
fn binary_search(value: &[i32], element: i32) -> Option<usize> {
    ...
    // 如果 start > end，则表明数组中不存在待查找元素，则查找失败，返回默认值 None
    return None;
}
...
/// 山脉数组中查找目标值
pub fn find_in_mountain_array(target: i32, array: Vec<i32>) -> Option<usize> {
    // 首先通过二分法找山顶
    let mut start = 0;
...
/// 二分查找
fn bs(target: i32, array: &Vec<i32>, mut start: usize, mut end: usize, rev: bool) -> Option<usize> {
    // 获取新的 target 值
